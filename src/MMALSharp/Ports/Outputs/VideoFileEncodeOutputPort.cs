...
namespace MMALSharp.Ports.Outputs
{
    ...
    public unsafe class VideoFileEncodeOutputPort : OutputPort
    {
        ...
        public VideoFileEncodeOutputPort(PortBase copyFrom)
        ...
        internal override void NativeOutputPortCallback(MMAL_PORT_T* port, MMAL_BUFFER_HEADER_T* buffer)
        {
            ...
            if (bufferImpl.CheckState() && MMALVideoFileEncoder.WorkingQueue != null)
            {
                if (MMALCameraConfig.Debug)
                {
                    MMALLog.Logger.Debug($"Putting output port buffer back into queue {MMALVideoFileEncoder.WorkingQueue}");
                }
